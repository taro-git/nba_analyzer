...
def seed_standings(session: Session) -> dict[int, list[RegularSeasonTeamStanding]]:
    ...
    return {2023: data_2023, 2024: data_2022}


@pytest.fixture
def seed_teams(session: Session) -> dict[int, TeamProperty]:
    """
    Team の初期データ
    team_id: 1~30
    """

    session.add_all([Team(id=i + 1) for i in range(30)])
    ...
    return {team.team_id: team for team in data}
